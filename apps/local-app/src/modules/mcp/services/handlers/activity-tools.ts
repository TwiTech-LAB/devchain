...
import {
  McpResponse,
  SessionContext,
  type ActivityStartParams,
  type ActivityFinishParams,
} from '../../dtos/mcp.dto';
import type { ActivityToolContext } from './activity-context';
...
function chatServiceUnavailable(): McpResponse {
  return {
    success: false,
    error: {
      code: 'SERVICE_UNAVAILABLE',
      message:
        'Activity operations require full app context (not available in standalone MCP mode)',
    },
  };
}

function catchChatUnavailable(error: unknown): McpResponse {
  if (error instanceof ServiceUnavailableError) {
    return chatServiceUnavailable();
  }
  throw error;
}

export async function handleActivityStart(
  ctx: ActivityToolContext,
  params: unknown,
): Promise<McpResponse> {
  const validated = params as ActivityStartParams;
  ...
  try {
    ...
    return catchChatUnavailable(error);
  }
}
...
export async function handleActivityFinish(
  ...
  const validated = params as ActivityFinishParams;

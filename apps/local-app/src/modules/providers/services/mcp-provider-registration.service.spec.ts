...
describe('McpProviderRegistrationService', () => {
  ...
  describe('resolveBinary', () => {
    ...
    it("falls back to 'which' lookup when binPath missing", async () => {
      accessMock.mockResolvedValue(undefined);
      fakeExecutor.enqueueResponse({ type: 'success', stdout: '/usr/bin/codex\n' });

      const provider = { ...baseProvider, name: 'codex', binPath: null };
      const result = await service.resolveBinary(provider);

      expect(fakeExecutor.calls[0].argv).toEqual(['which', 'codex']);
      expect(result.success).toBe(true);
  ...
  describe('registerProvider', () => {
    it('runs registration command with resolved binary', async () => {
      ...
      const result = await service.registerProvider(baseProvider, {
    ...
    it('returns failure when command errors', async () => {
      accessMock.mockResolvedValue(undefined);
      fakeExecutor.enqueueResponse({ type: 'failure', exitCode: 1, stderr: 'spawn failed' });
  ...
  describe('listRegistrations', () => {
    it('executes list command and returns normalized entries', async () => {
      ...
      const result = await service.listRegistrations(baseProvider);
      expect(result.success).toBe(true);
      ...
      expect(fakeExecutor.calls[0].argv).toEqual([baseProvider.binPath, 'mcp', 'list']);
      expect(fakeExecutor.calls[0].mode).toBe('pipe');
    });
  ...
  describe('removeRegistration', () => {
    it('executes remove command with adapter', async () => {
      ...
      const result = await service.removeRegistration(baseProvider, 'devchain');
      expect(result.success).toBe(true);

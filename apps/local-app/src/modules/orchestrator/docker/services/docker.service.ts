...
import { WorktreeMountDiscoveryService, type MountPlan } from './worktree-mount-discovery.service';
...
const DEFAULT_CONTAINER_DATA_PATH = `/home/node/.devchain`;
...
export class OrchestratorDockerService {
  constructor(
    private readonly mountDiscovery: WorktreeMountDiscoveryService,
    @Optional()
  ...
  async createContainer(config: CreateContainerConfig): Promise<ContainerInfo> {
    ...
    const mountPlan = await this.mountDiscovery.discoverMountPlan({
      worktreePath: config.worktreePath,
      worktreeName,
    });

    const envMap = this.buildEnvMap(config.env, worktreeName, mountPlan);
    const binds = this.buildBindMounts(config, mountPlan);
  ...
  private buildEnvMap(
    ...
    // Discovery-derived env (git identity, ENABLED_PROVIDERS, copilot tokens) merges
    // via ??= so explicit caller/user values always win. Iteration order matches the
    // plan's key insertion order — byte-identical to the pre-extraction precedence.
    for (const [key, value] of Object.entries(mountPlan.env)) {
      envMap[key] ??= value;
    }
    return envMap;
  }
  ...
  private buildBindMounts(config: CreateContainerConfig, mountPlan: MountPlan): string[] {

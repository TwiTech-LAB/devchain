...
describe('EpicsService', () => {
  ...
  describe('epic.assigned deprecated compatibility', () => {
    it('publishes epic.assigned (deprecated) when creating epic with agent', async () => {
      const epicWithAgent: Epic = {
        ...baseEpic,
        agentId: 'agent-1',
      };
    ...
    it('publishes epic.assigned (deprecated) on agent assignment and reassignment', async () => {
      // Assign agent (null → agent-1)
      storage.getEpic.mockResolvedValue(baseEpic);
      storage.updateEpic.mockResolvedValue({
        ...baseEpic,
        agentId: 'agent-1',
      ...
      await service.updateEpic(baseEpic.id, { agentId: 'agent-1' }, baseEpic.version);
      ...
      // Reassign agent (agent-1 → agent-2)
      storage.getEpic.mockResolvedValue({ ...baseEpic, agentId: 'agent-1', version: 2 });
      storage.updateEpic.mockResolvedValue({
        ...baseEpic,
        agentId: 'agent-2',
        version: 3,
      ...
      await service.updateEpic(baseEpic.id, { agentId: 'agent-2' }, 2);
